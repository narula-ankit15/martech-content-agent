...
from app.models import Asset, CampaignBrief, ProjectContext, WhatsAppBrief, WhatsAppDraft, WhatsAppLength

SYSTEM_PROMPT = """You are a real-estate marketing copywriter generating a WhatsApp/SMS message for a CRM campaign.
Rules:
...
- Use emojis where they fit the tone naturally (e.g. real-estate-relevant: 🏡 🔑 📍 ✨ 🎉) -- tasteful and not excessive, skip them entirely for a b2b_professional or internal_team audience.
...
separate:
  ...
  2. If there are 2+ discrete facts to state (price, possession date, a specific offer, unit sizes, etc.), a
     short lead-in line ("Here are the details:" or similar) followed by each fact on its own line as
...
Wrap 2-4 of the most important words/phrases per variant (a price, a deadline, a key benefit) in single asterisks
for WhatsApp bold, e.g. "starting from *Rs 87 Lakhs*" or "*early possession* available" -- never wrap a whole
sentence, and never use double asterisks or markdown headers (WhatsApp doesn't render those)."""
...
class WhatsAppContentAgent:
    ...
    def generate(
        ...
        *,
        ...
        prompt = self._build_prompt(campaign_brief, whatsapp_brief, context, assets)
    ...
    def revise(
        ...
        *,
        ...
        assets = (candidate_assets or []) if whatsapp_brief.image_required else []
        base_prompt = self._build_prompt(campaign_brief, whatsapp_brief, context, assets)
        prompt = (
