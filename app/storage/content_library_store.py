...
class ContentLibraryStore:
    ...
    @staticmethod
    def _random_template_id(channel_value: str) -> str:
        # E for email, WA for whatsapp -- matches the channel at a glance
        # without having to open the card.
        prefix = "E" if channel_value == "email" else "WA"
    ...
    def list_by_project(self, project_id: str, channel: Optional[Channel] = None) -> list[ContentLibraryEntry]:
        query = f"SELECT {', '.join(_COLUMNS)} FROM content_library WHERE project_id = ?"
